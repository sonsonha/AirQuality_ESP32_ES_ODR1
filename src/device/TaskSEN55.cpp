...
void SEN55_sensor()
{
    ...
    if (tb.connected())
    {
        publishData("telemetry", "pm1", String(pm1p0, 2));
        publishData("telemetry", "pm25", String(pm2p5, 2));
        publishData("telemetry", "pm4", String(pm4p0, 2));
...
void TaskSEN55(void *pvParameters)
{
    ...
    while (true)
    {
        SEN55_sensor(); // Đọc và gửi dữ liệu
